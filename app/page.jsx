...
const demoPosts = [
  ...
  { id: 6, platform: "instagram", type: "Story", title: "Q&A: Häufigste Fragen zu unserem Netzwerk", date: "2026-02-14T18:00:00", views: 0, likes: 0, comments: 0, shares: 0, done: false, status: "scheduled", timezone: "CET", createdAt: "2026-02-10", createdBy: "Dariel" },
  { id: 7, platform: "tiktok", type: "Video", title: "Vorher/Nachher: Agentur-Transformation", date: "2026-02-17T12:30:00", views: 0, likes: 0, comments: 0, shares: 0, done: false, status: "scheduled", timezone: "CET", createdAt: "2026-02-12", createdBy: "Dariel" },
...
function CreatePostModal({ onClose, onSubmit, isSubmitting, accounts }) {
  ...
  const handleSubmit = () => {
    ...
    if (thumbnailMode === "scrub" && thumbnailTimestamp !== null && selectedPlatforms.includes("tiktok")) {
      tiktokSettings = { video_cover_timestamp_ms: thumbnailTimestamp };
    }

    const cleanCollabs = collabs.map((c) => c.trim().replace(/^@/, "")).filter(Boolean);

    onSubmit({ content, platforms: platformsPayload, scheduledFor, publishNow: postNow, mediaItems, timezone, tiktokSettings, contentType, collaborators: cleanCollabs, customThumbnail: thumbnailMode === "upload" && customThumbnail?.url ? customThumbnail.url : null });
  };
  ...
  const handleCreatePost = async ({ content, platforms, scheduledFor, publishNow, mediaItems, timezone, tiktokSettings }) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        const tzShort = TIMEZONES.find((t) => t.value === timezone)?.short || "CET";
        const np = { id: data.id || Date.now(), platform: platforms[0]?.platform || "instagram", type: mediaItems?.length ? "Video" : "Post",
          title: content.substring(0, 60) + (content.length > 60 ? "..." : ""),
          date: scheduledFor || new Date().toISOString(), views: 0, likes: 0, comments: 0, shares: 0,
