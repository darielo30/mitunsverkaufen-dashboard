...
import {
  Check, Eye, Heart, MessageCircle, Share2,
  ...
  Bookmark, Copy, List, LayoutGrid, Minus, MousePointerClick, ArrowUpDown, Menu,
...
const darkTheme = {
  bg: "#121212", bgSoft: "#171717", card: "#171717", cardHover: "#1d1d1d",
  border: "#2E2E2E",
  glass: "#171717", glassHover: "#1d1d1d", glassBorder: "#2E2E2E",
  glassStrong: "#171717",
...
// ── Offizielle Plattform-Markenzeichen (Simple Icons, CC0) ──────
// Echte Logo-Glyphen statt Annäherungen – ein Pfad pro Plattform, als
// gefülltes SVG mit `currentColor`/`color`-Prop, damit sie sich überall wie
// bisherige lucide-Icons einsetzen lassen (`<Icon size={n} color={c} />`).
...
function makeGlyphIcon(pathKey) {
  return function GlyphIcon({ size = 24, color = "currentColor" }) {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" fill={color}>
        <path d={PLATFORM_GLYPH_PATHS[pathKey]} />
      </svg>
    );
  };
}

// Lokale Ersatz-Komponenten unter den vertrauten Namen – jede bisherige
// Stelle, die `<Instagram .../>`, `<TikTokIcon .../>` etc. rendert, bekommt
// dadurch automatisch die echte Marken-Glyphe statt der lucide-Näherung.
const TikTokIcon = makeGlyphIcon("tiktok");
const Instagram = makeGlyphIcon("instagram");
const Youtube = makeGlyphIcon("youtube");
const Facebook = makeGlyphIcon("facebook");
const Linkedin = makeGlyphIcon("linkedin");

// Farbig hinterlegtes Badge (abgerundetes Quadrat + weiße Glyphe), exakt wie
// Zernios Post-Karten die Plattform je Beitrag kennzeichnen. Optional als
// Link gerendert, wenn eine echte Post-URL vorliegt.
...
function PlatformBadge({ platform, size = 20, href, title }) {
  const path = PLATFORM_GLYPH_PATHS[platform];
  const bg = PLATFORM_BADGE_BG[platform] || C.muted;
  const glyph = (
  ...
  const style = {
    width: size, height: size, borderRadius: "30%", background: bg, flexShrink: 0,
    display: "flex", alignItems: "center", justifyContent: "center",
    opacity: href ? 1 : 0.45, cursor: href ? "pointer" : "default",
    transition: "transform 0.12s, box-shadow 0.12s",
  };
  if (!href) return <div title={title} style={style}>{glyph}</div>;
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" title={title || "Beitrag auf der Plattform ansehen"}
      onClick={(e) => e.stopPropagation()}
      onMouseOver={(e) => { e.currentTarget.style.transform = "scale(1.12)"; e.currentTarget.style.boxShadow = `0 2px 10px rgba(0,0,0,0.35)`; }}
...
function Sidebar({ activeTab, onTabChange, unreadCount, isDarkMode, onToggleTheme, isOpen, onClose }) {
  ...
  const SB = {
    bg: isDarkMode ? "#171717" : "rgba(255,255,255,0.75)",
    activeBg: C.accentGlow,
    hoverBg: isDarkMode ? "rgba(255,255,255,0.06)" : "#EEEEFF",
    text: isDarkMode ? "#A3A3A3" : "#6B7280",
    hoverText: isDarkMode ? "#FAFAFA" : "#111827",
    activeText: isDarkMode ? "#F5F7FA" : "#111827",
    border: isDarkMode ? "#2E2E2E" : "#E5E7EB",
          ...
          return (
            <div key={item.key}>
              <button
                ...
                onMouseOver={(e) => { if (!isParentActive && !item.disabled) { e.currentTarget.style.background = SB.hoverBg; e.currentTarget.style.color = SB.hoverText; } }}
                onMouseOut={(e) => { if (!isParentActive && !item.disabled) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = SB.text; } }}
              >
                <item.icon size={16} />
                  ...
                  {item.children.map((child) => {
                    ...
                    return (
                      <button key={child.key} onClick={() => { onTabChange(child.key); onClose?.(); }} style={subItemStyle(childActive)}
                        onMouseOver={(e) => { if (!childActive) { e.currentTarget.style.background = SB.hoverBg; e.currentTarget.style.color = SB.hoverText; } }}
                        onMouseOut={(e) => { if (!childActive) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = SB.text; } }}>
                        {child.label}
                      </button>
                    );
                  })}
              ...
              <div className="glass-panel" onClick={() => setSelectedPost(isSelected ? null : post)}
                ...
                <div style={{ display: "flex", gap: SPACE.xl, padding: 14, flex: 1, minHeight: 0 }}>
                  <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
                    ...
                    {/* Plattform-Icons – klickbar, führt direkt zum Beitrag auf der Plattform */}
                    <div style={{ display: "flex", gap: SPACE.sm, marginBottom: 9, alignItems: "center", height: 20 }}>
                      {postPlats.map((plat) => {
                        const url = post.postUrls?.[plat];
                        const label = PLATFORM_META[plat]?.label || plat;
                        return (
                          <PlatformBadge key={plat} platform={plat} size={20} href={url}
                            title={url ? `${label} – Beitrag ansehen` : `${label} (noch kein Link verfügbar)`} />
                        );
                      })}
                    </div>
