...
import {
  ...
  Globe, Play, Pause, SkipForward, SkipBack, Scissors
} from "lucide-react";
import {
...
function ThumbnailPicker({ videoFile, onSelect, selectedTimestamp }) {
  ...
  useEffect(() => {
    ...
    video.onloadedmetadata = () => {
      ...
      const count = Math.min(6, Math.max(3, Math.floor(video.duration / 2)));
      const times = [];
      for (let i = 0; i < count; i++) {
        times.push((video.duration / (count + 1)) * (i + 1));
      }
      ...
      video.onseeked = () => {
        ...
        frames.push({ time: times[idx], dataUrl: canvas.toDataURL("image/jpeg", 0.8), timeMs: Math.round(times[idx] * 1000) });
        idx++;
        captureFrame();
      };
  ...
  // Capture custom frame from scrubber
  const captureCustomFrame = () => {
    ...
    setCustomFrame({ time: videoRef.current.currentTime, dataUrl, timeMs });
    onSelect(timeMs);
  };
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      {thumbnails.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          {thumbnails.map((thumb, i) => (
            <button key={i} onClick={() => { onSelect(thumb.timeMs); setCustomFrame(null); }} style={{
              padding: 0, border: selectedTimestamp === thumb.timeMs && !customFrame ? `2px solid ${C.red}` : `2px solid ${C.border}`,
              borderRadius: 8, cursor: "pointer", background: "transparent", overflow: "hidden", position: "relative",
              opacity: selectedTimestamp === thumb.timeMs && !customFrame ? 1 : 0.7, transition: "all 0.2s",
            }} onMouseOver={(e) => e.currentTarget.style.opacity = "1"}
               onMouseOut={(e) => { if (selectedTimestamp !== thumb.timeMs || customFrame) e.currentTarget.style.opacity = "0.7"; }}>
              <img src={thumb.dataUrl} alt={`Frame ${i + 1}`} style={{ width: 96, height: 54, display: "block", objectFit: "cover" }} />
              <div style={{ position: "absolute", bottom: 2, right: 4, fontSize: 10, color: "#fff", fontWeight: 700, textShadow: "0 1px 3px rgba(0,0,0,0.8)" }}>
                {formatTime(thumb.time)}
              </div>
              {selectedTimestamp === thumb.timeMs && !customFrame && (
                <div style={{ position: "absolute", top: 2, left: 4, width: 16, height: 16, borderRadius: 4, background: C.red, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {!isReady && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: 12, fontSize: 13, color: C.muted }}>
          <Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> Thumbnails werden generiert...
        </div>
      )}

      {/* Custom frame scrubber */}
      <div style={{ background: C.bg, borderRadius: 10, padding: 12, border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: C.dimmed, marginBottom: 8 }}>Oder eigenen Frame wählen:</div>
        <video
          ref={videoRef}
          src={videoFile.file ? URL.createObjectURL(videoFile.file) : ""}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
          <button onClick={() => { if (videoRef.current) videoRef.current.currentTime = Math.max(0, videoRef.current.currentTime - 1); }}
            style={{ width: 28, height: 28, borderRadius: 6, background: C.card, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <SkipBack size={12} color={C.muted} />
          </button>
          ...
          <button onClick={() => { if (videoRef.current) videoRef.current.currentTime = Math.min(duration, videoRef.current.currentTime + 1); }}
            style={{ width: 28, height: 28, borderRadius: 6, background: C.card, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <SkipForward size={12} color={C.muted} />
          </button>
          <span style={{ fontSize: 11, color: C.dimmed, fontVariantNumeric: "tabular-nums", minWidth: 40 }}>{formatTime(currentTime)}</span>
          <button onClick={captureCustomFrame} style={{
            display: "flex", alignItems: "center", gap: 4, padding: "5px 12px", borderRadius: 6,
            background: C.red, border: "none", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
          }}>
            <Scissors size={12} /> Auswählen
          </button>
        </div>
        {customFrame && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8, padding: "6px 8px", background: C.redGlow, borderRadius: 8 }}>
            <img src={customFrame.dataUrl} alt="Custom thumbnail" style={{ width: 64, height: 36, borderRadius: 4, objectFit: "cover" }} />
            <div style={{ fontSize: 12, color: C.red, fontWeight: 600 }}>Eigenes Thumbnail bei {formatTime(customFrame.time)}</div>
            <Check size={14} color={C.red} />
          </div>
        )}
      </div>
    </div>
              ...
              {[{ key: "instagram", label: "Instagram", icon: Instagram, color: C.instagram }, { key: "tiktok", label: "TikTok", icon: Music, color: C.tiktok }].map((p) => {
                ...
                return (
                  <button key={p.key} onClick={() => setPlatforms({ ...platforms, [p.key]: !platforms[p.key] })} style={{
                    ...
                    {!account && <span style={{ fontSize: 10, color: C.yellow }}>!</span>}
                  </button>
                );
              })}
            </div>
            {accounts.length === 0 && (
              <div style={{ fontSize: 12, color: C.yellow, marginTop: 6 }}>Keine Accounts verbunden – verbinde Instagram/TikTok in deinem Late Dashboard</div>
            )}
          </div>
